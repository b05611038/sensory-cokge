...
from sensory_cokge.models import RoBERTa_NAME
...
def main():
    ...
    parser.add_argument('--model_select', type = str, default = RoBERTa_NAME,
            help = 'The BERT version you want to choose.')
    parser.add_argument('--model_name', type = str, default = 'finetuned-' + RoBERTa_NAME,
    ...
    tokenizer = RobertaTokenizer.from_pretrained(args.model_select)
    def preprocess_function(examples):
        inputs = [f"{examples['text0'][i]} </s> {examples['text1'][i]} </s> " + \
                  f"{examples['text2'][i]} </s> {examples['text3'][i]} </s> " + \
    ...
    model = RobertaForSequenceClassification.from_pretrained(args.model_select,
                                                             num_labels = 5).to(device)

    print('Start finetune RoBERTa ...')
    ...
    model.roberta.save_pretrained(base_model_path)

    print('Finish RoBERTa fine-tuning.')
    return None
